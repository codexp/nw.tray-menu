...
class CustomTrayMenu {
    constructor(windowPath, trayIcon, windowOptions) {
        ...
        this.trayIcon = trayIcon || 'public/img/tray.png';
        this.menuWindowPath = windowPath || '../view/tray-menu.html';
    ...
    $(selector) {
        if ('function' === typeof selector) {
            return this.trayMenu.window.document.addEventListener('DOMContentLoaded', selector);
        }
        return this.trayMenu.window.document.querySelector(selector);
    }
    ...
    _initMenuWindow() {
        ...
        this.trayMenu.on('document-end', function () {
            ...
            windowOptions.height = this.getHeight();
            this.trayMenu.window.resizeTo(windowOptions.width, windowOptions.height)
        }.bind(this));
    ...
    getHeight() {
        return this.trayMenu.window.document.body.offsetHeight;
    }
    ...
    translate(pos) {
        if (process.platform == 'darwin') {
            ...
            // for windows can not make exact position, because we have position of click. Just move it 5px up
